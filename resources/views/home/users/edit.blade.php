...
@section('content')
    ...
    <main class="main" role="main">
        ...
        <form class="form" method="POST" action="{{url('/home/users/')}}/{{$user->id}}">
            <input type="hidden" name="user_id" value="{{$user->id}}">

            {{ csrf_field() }}

            <header class="">
                <h2 style="margin-bottom: 50px; text-align: center;">Личные данные: ID {{ $user->id }}</h2>
            </header>

            <div class="form__item{{ $errors->has('name') ? ' has-error' : '' }}">
            ...
            <div class="form__item{{ $errors->has('email') ? ' has-error' : '' }}">
            ...
            <div class="form__item{{ $errors->has('first_name') ? ' has-error' : '' }}">
            ...
            <div class="form__item{{ $errors->has('last_name') ? ' has-error' : '' }}">
            ...
            <div class="form__item{{ $errors->has('birthday') ? ' has-error' : '' }}">
                <label for="birthday">Дата рождения</label>
                <input id="birthday" class="form-control pick_birthday" type="text" name="birthday"
                       value="{{$user->birthday}}" placeholder="Введите дату" required>
                @if ($errors->has('birthday'))
                    <p>{{ $errors->first('birthday') }}</p>
                @endif
            </div>

            <div class="form__item{{ $errors->has('terra_id') ? ' has-error' : '' }}">
            ...
            <div class="form__item{{ $errors->has('status') ? ' has-error' : '' }}">
